...
module.exports = {
  ...
  loginData: (req, res) => {
    ...
    let scriptQuery = `select * from db_sepaket.users where email = ${db.escape(
      req.body.email
    )} and password = '${password}';`;
    console.log(
      `select * from db_sepaket.users where email = ${db.escape(
        req.body.email
      )} and password = '${password}';`
    );
    ...
    db.query(scriptQuery, (err, result) => {
      ...
      if (result[0]) {
        ...
        let scriptQuery2 = `select * from db_sepaket.users where username = ${db.escape(
          req.body.email
        )} and password = '${password}';`;
        console.log(
  ...
  addData: (req, res) => {
    ...
    let scriptQuery = `insert into db_sepaket.users value (null, ${db.escape(
      ...
      password
    )},null,null,null,null,null,'user','unverified');`;
    db.query(scriptQuery, (err, result) => {
      if (err) res.status(500).send(err);
